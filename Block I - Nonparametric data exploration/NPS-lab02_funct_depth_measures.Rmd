...
lines(grid, median.mbd$values, col="black", lwd=5)
library(DepthProc)
tukey.depth=depth(u=f_data$values,method='Tukey')
tukey.deepest.idx = which(tukey.depth==max(tukey.depth))
lines(grid, f_data$values[tukey.deepest.idx[1],], col="red", lwd=5)
...
### Spearman's correlation index for functional data
...
We recall the definition of the (sample) Modified Epigraph Index (MEI):
$$
MEI(f_i) = N^{-1} \sum_{i\neq j} \lambda ( \{t \in \mathcal{I}: f_j(t) \geq f_i(t) \})
$$
that is: for the $i$th function, the mean, with respect to all other functions $j$  of the lebesgue measure, ie. total time or total length of subset of domain, such that the $j$th function has a higher value than the $i$ one.

For the Hypograph index, we just change the sign: we count the mean "time" other functions have a lower value than the $i$th one.

The sample  $IL_{n}\operatorname{grade}(x_i)$ for the $i$th corresponds to the `MEI`, except that the divisor of the sum also takes into account the length of the interval over which we measure the functions. Same goes for $SL_{n}\operatorname{grade}(x_i)$.




As an example, let us use them to compute the Spearman's correlation index
